...
if ($max > 0) { // Pastikan ada baris data sebelum mencoba mengaksesnya
    $minDate = $row[0]['order_date'];
    $maxDate = $row[$max -1]['order_date'];
} else {
    // Jika tidak ada data, set tanggal default atau tampilkan pesan
    // Anda bisa mengatur tanggal saat ini, atau tanggal awal/akhir yang wajar
    $minDate = date('Y-m-d'); // Contoh: tanggal hari ini
    $maxDate = date('Y-m-d'); // Contoh: tanggal hari ini
    // Atau Anda bisa mengarahkan ke halaman lain, menampilkan pesan error, dll.
}
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <?php if (!empty($rowTransOrderDetail)): ?>
